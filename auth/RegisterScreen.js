...
export default function RegisterScreen({ navigation }) {
  ...
  const handleRegister = async () => {
    ...
    if (!strongEnough) {
      setBanner({ type: 'error', msg: 'Password must be at least 6 characters.' });
      return;
    }
    ...
    try {
      // ✅ 把用户名放到 user_metadata，等用户验证并首次登录后再写入 profiles（见下方说明）
      const { data, error } = await supabase.auth.signUp({
        email: e,
        password: p,
        options: {
          emailRedirectTo: 'https://reset-password-page-one.vercel.app', // 或你的 App 深链 myapp://auth-callback
          data: { username: u },
        },
      });

      if (error) {
        // 常见：User already registered、password weak 等
        setBanner({ type: 'error', msg: error.message || 'Registration failed. Please try again.' });
        return;
      }

      // Supabase 默认需要邮件验证，此时通常没有 session
      setBanner({
        type: 'success',
        ...
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.inputRow}>
              <Ionicons name="at" size={20} color={TEXT_MUTED} style={styles.inputIcon} />
              <TextInput
            ...
            <View style={styles.inputRow}>
              ...
              <TouchableOpacity onPress={() => setShowPwd((s) => !s)} style={styles.eyeBtn}>
                <Ionicons name={showPwd ? 'eye-off' : 'eye'} size={22} color={TEXT_MUTED} />
              </TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  helperText: { color: TEXT_MUTED, fontSize: 12, marginTop: -6, marginBottom: 8, marginLeft: 4 },
  primaryBtn: {
    height: 50,
  ...
  footerNote: { textAlign: 'center', color: '#9ca3af', fontSize: 12, marginTop: 14 },
});
